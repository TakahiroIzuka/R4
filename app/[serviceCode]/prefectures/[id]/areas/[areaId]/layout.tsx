...
interface AreaLayoutProps {
  ...
  params: Promise<{ serviceCode: string; id: string; areaId: string }>
}
...
export default async function AreaLayout({
  ...
  const { serviceCode, id: prefectureId, areaId } = await params
  ...
  const breadcrumbItems = [
    ...
    { label: prefectureData?.name || '', href: `/${serviceCode}/prefectures/${prefectureId}` },
    { label: area.name }
  ]
