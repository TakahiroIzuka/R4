...
export default async function DetailPage({ params }: DetailPageProps) {
  ...
  const { facility, error } = await fetchFacilityById(id, serviceCode)
  ...
  const genres = config?.genres as Record<string, { color: string; lineColor: string }> | undefined
  const genreColor = (genreCode && genres?.[genreCode]?.color) || config?.color || 'rgb(165, 153, 126)'

  // Default logo URL for service
  const defaultLogoUrl = `/${serviceCode}/icon.png`
