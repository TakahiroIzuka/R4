...
Vercel は GitHub リポジトリと連携しており、`main` ブランチへの merge で自動デプロイされます。
...
Vercel ダッシュボード（Settings > Environment Variables）で変更します。変更後は**再デプロイが必要**です。
...
> **注意**: 環境変数の変更は再デプロイしないと反映されません。ダッシュボードで変更後、Deployments から再デプロイを実行してください。

#### Supabase（Edge Functions）

Supabase ダッシュボードまたは CLI で管理します。変更後は**再デプロイ不要**で即時反映されます。

**ブラウザから変更する場合:**

1. [Supabase ダッシュボード](https://supabase.com/dashboard) にログイン
2. 対象プロジェクトを選択
3. 左メニューの **Edge Functions** を開く
4. **Manage Secrets** をクリック
5. シークレットを追加・編集・削除する
